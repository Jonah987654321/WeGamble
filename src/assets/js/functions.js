...
function animateCountUp (el, duration, start, countTo) {
	...
	const counter = setInterval( () => {
		...
		const currentCount = start + Math.round( (countTo-start) * progress );

		// If the current count has changed, update the element
		if ( parseInt( el.innerHTML, 10 ) !== currentCount ) {
			el.innerHTML = formatCurrency(currentCount)+"€";
		}
...
// The animation function, which takes an Element
function animateCountDown (el, duration, start, countTo) {
    const animationDuration = duration;
    const frameDuration = 1000 / 60;
    const totalFrames = Math.round( animationDuration / frameDuration );
    let frame = 0;
	...
	const counter = setInterval( () => {
		...
		const progress = easeOutQuad( frame / totalFrames );
		// Use the progress value to calculate the current count
		const currentCount = start-Math.round( (start-countTo) * progress );
		// If the current count has changed, update the element
